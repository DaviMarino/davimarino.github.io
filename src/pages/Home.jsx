...
// CORREÇÃO CRÍTICA: ExternalLink adicionado aqui para evitar a tela branca
import { GitBranch, BarChart2, Layers, ExternalLink } from 'lucide-react';
...
const Home = ({ setSelectedProject }) => {
  ...
  return (
    <>
      {/* Hero Section */}
      <section id="sobre" className="pt-24 pb-20 px-6 relative overflow-hidden">
        <div className="container mx-auto grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="z-10 flex flex-col justify-center">
            ...
            <p className="text-lg mb-10 leading-relaxed max-w-lg" style={{ color: theme.secondaryText }}>
                {personalInfo.role}. Especialista em Power BI, Python e estratégias de governança de dados.
            </p>
            
            {/* --- ÁREA DOS CARDS SOCIAIS --- */}
            <div className="flex flex-col md:flex-row gap-6 items-start">
                
                {/* Card Linkedin Embed (Oficial) */}
                <div className="w-[245px]">
                    <div 
                        ...
                        data-theme="dark" 
                        data-type="VERTICAL" 
                        data-vanity="davimarino" 
                        data-version="v1"
                    >
                        <a className="badge-base__link LI-simple-link" href="https://br.linkedin.com/in/davimarino?trk=profile-badge"></a>
                    </div>
                </div>

                {/* Card GitHub Customizado (API) */}
          ...
          {/* Mockup do Código */}
          <div className="relative rounded-lg shadow-2xl overflow-hidden border font-mono text-sm leading-6 sticky top-24" style={{ backgroundColor: theme.bg, borderColor: theme.border }}>
          ...
          <div className="space-y-12">
            {experienceData.map((e, i) => (
              <div key={i} className="group relative pl-8 border-l-2" style={{ borderColor: theme.border }}>
                <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full border-4 transition-all group-hover:scale-125" style={{ backgroundColor: theme.func, borderColor: theme.bg }}></div>
                <div className="p-6 rounded-lg border bg-[#161b22] border-[#30363d] hover:border-opacity-100 transition-all">
                  <div className="flex flex-col md:flex-row justify-between mb-4 md:items-center">
                    ...
                    <span className="text-xs font-mono px-2 py-1 rounded border mt-2 md:mt-0 w-fit" style={{ backgroundColor: theme.bg, borderColor: theme.border, color: theme.secondaryText }}>{e.period}</span>
                  </div>
                  <div className="text-slate-400 mb-6 text-sm leading-relaxed space-y-3">
                    {e.content.map((block, idx) => {
                      ...
                      if (block.type === 'header') return <h4 key={idx} className="font-bold mt-4 mb-2 text-slate-200">{block.text}</h4>;
                      if (block.type === 'list') return <ul key={idx} className="space-y-2 ml-1">{block.items.map((item, itemIdx) => <li key={itemIdx} className="flex gap-2"><span style={{ color: theme.func }}>✔</span><span>{item}</span></li>)}</ul>;
                      return null;
                    })}
                  </div>
                  <div className="flex flex-wrap gap-2 pt-4 border-t border-[#30363d]">
                    {e.tech.map((t, j) => <span key={j} className="text-xs px-2 py-1 rounded font-mono border" style={{ backgroundColor: theme.bg, borderColor: theme.border, color: theme.keyword }}>{t}</span>)}
                  </div>
                </div>
              </div>
            ))}
      ...
      <section id="projetos" className="py-20 border-t" style={{ borderColor: theme.border, backgroundColor: "#0d1117" }}>
        <div className="container mx-auto px-6">
          <div className="flex items-center justify-between mb-12">
            ...
            {/* O ExternalLink é usado nesta linha abaixo */}
